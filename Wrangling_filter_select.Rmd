...
```
After filtering i found that there is 8 Batman movies.
...
movies %>%
  ...
  drop_na(budget) %>%
```
The code does not work because it is in the wrong order.
...
movies %>%
  arrange(year) 
```
after running the codes I found out that the longest movie was The Cure For Insomnia and found out the oldest movie was The Blacksmith Scene (1983).
...
```
After running the code above I found that there are 3 harry potter movies (Harry Potter and the Chamber of Secrets	, Harry Potter and the Prisoner of Azkaban,	
Harry Potter and the Sorcerer's Stone). The movie with the highest rating was Harry Potter and the Prisoner of Azkaban, the movie with the highest votes was Harry Potter and the Sorcerer's Stone with 46741 votes 
...
movies %>%
  ...
  arrange(desc(budget))
```
The highest budget for a comedy or animation was Wild Wild West.
